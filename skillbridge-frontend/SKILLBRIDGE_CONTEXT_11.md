...
- `/admin/login` — Username + password login for admins **and instructors** (dual-role, temporary for dev). Routes to `/admin/dashboard` or `/instructor/dashboard` based on credentials. Dummy creds stored here until Week 3 API. — ✅ built (`AdminLogin.jsx`)
- `/instructor/pending` — Holding screen shown to instructors whose account is awaiting admin approval. Has "Check Status Again" (reload) and "Cancel & Sign Out" buttons. — ✅ built (`InstructorPending.jsx`)
...
- `/admin/users` — tab switcher: Students table + Instructors table + Email-first Add Instructor modal. Uses shared **User Detail Modal** to view profiles, see assessment status (Retake toggling), and inline edit Instructor details (pencil icon). — ✅ built + updated (`AdminUsers.jsx`)
...
- `AdminUsers.jsx` — "Export" button on the students table
...
| "Export" button on students table             | `AdminUsers.jsx`          | ⬜ not built |
...
- **Assessment Retake Workflow (`StudentDashboard`, `EnrolledStudents`, `InstructorDashboard`, `AdminUsers`):**
...
- **Instructor Pending Screen (`InstructorPending.jsx`):**
  - Added `/instructor/pending` route shown when an instructor logs in but hasn't been approved yet.
  - Has "Check Status Again" (reload) and "Cancel & Sign Out" buttons.
  - Note: hardcoded email `faculty@dnsc.edu.ph` in the footer — replace with real user email in Week 3.
- **Dummy credentials in `AdminLogin.jsx` (remove in Week 3):**
  - `admin` / `admin01031` → routes to `/admin/dashboard`
  - `instructor` / `instructor01031` → routes to `/instructor/dashboard`
  - The admin login page doubles as instructor login temporarily — split into separate auth flows when real JWT is wired.

**Affected files this session:** EnrolledStudents.jsx, StudentDashboard.jsx, AdminUsers.jsx, InstructorDashboard.jsx
...
│   │   ├── AdminLogin.jsx     ✅ Admin + instructor credentials login (dual-role until Week 3 API)
│   │   │                         Dummy creds: admin/admin01031, instructor/instructor01031 — REMOVE in Week 3
│   │   └── InstructorPending.jsx ✅ Pending approval holding screen for instructors
│   ├── student/
...
│       ├── AdminUsers.jsx           ✅ tab switcher: Students + Instructors tables + email-first Add Instructor modal
│       │                               Clickable users via UserDetailModal. (Note: file was previously misnamed AdminStudents.jsx in docs)
│       └── AdminNotifications.jsx   ✅ notification centre: All/Unread/Read tabs, mark-as-read, click-to-navigate
...
- [x] Login page built
- [x] Admin login page built (AdminLogin.jsx — also handles instructor login temporarily)
- [x] Instructor pending page built (InstructorPending.jsx)
...
- [x] Admin pages built (static) — dashboard, companies, users, notifications
- [x] AdminNotifications page built — filter tabs, mark-as-read, click-to-navigate
...
- [x] `AdminUsers.jsx` — export button
