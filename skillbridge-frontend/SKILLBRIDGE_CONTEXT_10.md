...
- Final placement decisions remain with instructors (system is decision *support*, not automatic assignment)
...
- `/instructor/students` — enrolled students list with **batch dropdown selection (Active/Archived cohorts)**. Enroll modal has two tabs: Excel upload and manual entry. **Manual entry no longer asks for full name — name is automatically extracted from the DNSC email** (`lastname.firstname@dnsc.edu.ph` → `Firstname Lastname`) with a live preview card. Duplicate ID/email detection. Green toast on success. Uses the shared **Student Detail Modal** for per-student view and actions. — ✅ built + updated
...
- **Email-Derived Name Automation (`AddInstructorModal` & `EnrollModal`):**
  - Eliminated manual full-name entry. Typing `john.doe@dnsc.edu.ph` live-previews "Doe John", which can be confirmed or edited. 
...
| Student name from email | Extract from DNSC email `lastname.firstname@dnsc.edu.ph` | DNSC emails already encode the student's name — no need to ask twice; reduces typos |
| Manual enroll fields | Student ID + Email only (name derived, course selected) | Simpler form, fewer errors, name always matches institutional email format |
